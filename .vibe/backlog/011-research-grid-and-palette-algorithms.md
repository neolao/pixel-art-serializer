...
Sample images supplied so far live in `fixtures/candidate-images/` (three confirmed pixel-art images — `pixel-art-cat.png`, `pixel-art-cat-grid-drawing.png`, `pixel-art-mario-sprite.png` — plus five small icons of undetermined pixel-art status). The user has said more will be sent to complete the set before implementation starts — check this folder for the latest set when starting `/vibe:feature 011`.
